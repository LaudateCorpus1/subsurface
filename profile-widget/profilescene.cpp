...
ProfileScene::ProfileScene(double dpr, bool printMode, bool isGrayscale) :
	...
	diveComputerText(new DiveTextItem(dpr, 1.0, Qt::AlignRight | Qt::AlignTop)),
...
void ProfileScene::updateAxes(bool instant)
{
	...
	const double minProfileFraction = 0.5;
		...
		{ heartBeatAxis, 100.0, prefs.hrgraph },
		{ percentageAxis, 50.0, prefs.percentagegraph },
		{ gasYAxis, 100.0, ppGraphsEnabled() },
	...
	for (const VerticalAxisLayout &l: secondaryAxes) {
		...
		l.axis->animateChangeLine(QRectF(leftBorder, bottomBorder, width, l.height), animSpeed);
	}
	...
	cylinderPressureAxis->animateChangeLine(QRectF(leftBorder, topBorder + 0.24 * height, width, 0.80 * height), animSpeed);
}
